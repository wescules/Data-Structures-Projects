...
for casenum in `seq 1 1 $casecount`; do
    checkIfFileExists "a$casenum.txt"
    checkIfFileExists "b$casenum.txt"
...
for casenum in `seq 1 1 $casecount`; do
    ...
    timeout -k $timeout $timeout ./$filename "operation=add;A=a$casenum.txt;B=b$casenum.txt;result=$casenum.out;recursive=n;storage=linkedlist" 1>$casenum.stdout 2>$casenum.stderr 
    if [ $? -ne 0 ]; then
